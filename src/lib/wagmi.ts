...
export const config = createConfig({
  chains,
  // injected FIRST: browser-extension wallets (MetaMask/Rabby/Brave…) get a
  // native one-click connect AND reconnect instantly + silently on every
  // page load — this is what makes "connect once, stays connected across
  // all pages" actually seamless. walletConnect stays for mobile/QR.
  // Order matters: connectors[0] = injected, so useConnectWallet prefers it
  // when a browser wallet is present.
  connectors: [
