...
/**
 * Sum of all DIRECT ERC-20 transfers to a recipient address for a
 * specific token on the current chain. Used by /fund's progress bar
 * for direct-deposit (non-pool) donations.
 *
 * Reads standard ERC20 Transfer(indexed from, indexed to, value) events
 * filtered by indexed `to == recipient` on the token contract address.
 * Sums `value` per log. Same 30s polling + 50k-block lookback as the
 * pool-reveal version. Same graceful-degradation on RPC failures.
 *
 * NOTE: native ETH donations are NOT tracked here (no events) — direct
 * transfers via sendTransaction are invisible to log queries. For v1
 * we limit /fund to ERC-20 donations only; native donation tracking
 * would need an indexer or alchemy_getAssetTransfers.
 */
export function useDirectDonationsToRecipient(
  ...
  const fetchTransfers = useCallback(async () => {
    if (!publicClient || !tokenAddress || !recipient) {
    ...
    try {
      ...
      setState({ totalRaised: total, count: logs.length, isLoading: false });
    } catch (e) {
      console.debug('[useDirectDonationsToRecipient] getLogs failed:', e);
      setState({ totalRaised: 0n, count: 0, isLoading: false });
    }
  }, [publicClient, tokenAddress, recipient]);

  useEffect(() => {
    void fetchTransfers();
    const id = setInterval(() => { void fetchTransfers(); }, 30_000);
    return () => clearInterval(id);
  }, [fetchTransfers]);

  return { ...state, refetch: () => { void fetchTransfers(); } };
}
