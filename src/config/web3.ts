...
export const WEB3_CONFIG = {
  ...
  // CyrusFundraise — non-custodial campaign registry (on-chain ids +
  // accurate raised counter, 0.1%/$0.10 fee). Deployed to Sepolia only for
  // now (prototype-one-first); other testnets get their own env slots when
  // replicated. Empty slot => the fundraise-contract path is unavailable on
  // that chain and the UI falls back to the legacy direct-transfer flow.
  CYRUSFUNDRAISE_ETH_CONTRACT: _IS_MAINNET
    ? import.meta.env.VITE_CYRUSFUNDRAISE_ETH_MAINNET_CONTRACT
    : import.meta.env.VITE_CYRUSFUNDRAISE_ETH_TESTNET_CONTRACT,
