...
import {
  Send, Coins, ShieldCheck, AlertTriangle, Check, ExternalLink, Copy, Wallet,
} from "lucide-react";
import {
  usePool,
  usePoolBucketSizes,
...
export const PayForm = ({ activeChain }: PayFormProps) => {
  ...
  return (
    <Card className="p-6 bg-gradient-card backdrop-blur border-vault-primary/30 space-y-5">
      ...
      {/* Primary CTA — when disconnected this same button connects the
          wallet (one affordance, no separate connector card on the page). */}
      {!isConnected ? (
        <Button
      ...
      {/* Onramp — multi-provider chooser (Onramper / Ramp / Paybis / Unlimit).
          Each opens the provider's hosted onramp; gated per-provider on its
          own env key (see OnrampSection). */}
      <OnrampSection
