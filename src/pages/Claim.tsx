...
import { Lock, ExternalLink, AlertTriangle, Check, ChevronLeft, Clock, Wallet } from "lucide-react";
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-4">
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Use the "Connect wallet to claim" button below. You don't have to be the recipient —
                anyone with the URL can broadcast this reveal. Whoever sends the tx pays gas; the
                bucket lands at the recipient address regardless.
              </p>
            </div>
          </div>
        </Card>
      ...
      {claimState === "already-spent" ? (
        ...
        <Card className="p-6 bg-gradient-card backdrop-blur border-vault-primary/30">
          {!isConnected ? (
            <Button onClick={connectWallet} className="w-full bg-gradient-vault text-primary-foreground shadow-vault hover:opacity-90">
              <Wallet className="w-4 h-4 mr-2" /> Connect wallet to claim
            </Button>
          ) : (
