...
- [ ] **Alchemy / Infura mainnet RPC URL** in deployer's `.env` (not the
  testnet endpoints).
- [ ] **Worker rebuilt + redeployed** with mainnet contract addresses in
  `.env` BEFORE the frontend's "mainnet" toggle flips. Run
  `../cyrusthegreat-worker/deploy.sh` — it builds with `.env` (baking the
  vars) then `wrangler deploy`. Since the 2026-07-09 Pages→Worker migration
  there is NO Cloudflare-side env to sync and no dashboard retry; vars bake
  at build time. (Legacy Pages path: `../tools/cf-sync-env.sh` then retry.)
...
5. Frontend: flip `VITE_ENABLE_MAINNET=true` in `.env`, commit the code change, push to main
6. Rebuild + redeploy the Worker: `../cyrusthegreat-worker/deploy.sh` — builds with the
   updated `.env` (bakes the mainnet contract addresses AND the flag), then `wrangler deploy`.
   Confirm the live `/assets/index-*.js` hash changed.
7. NOTE (changed 2026-07-09): pushing to `main` no longer auto-deploys — the Pages
   auto-deploy is gone. The Worker deploy is the explicit `deploy.sh` run in step 6.
