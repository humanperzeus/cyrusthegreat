...
# Then rebuild + redeploy the Worker (vars bake at build time):
../cyrusthegreat-worker/deploy.sh
# Since the 2026-07-09 Pages→Worker migration there is NO Cloudflare dashboard
# env to sync — editing .env only takes effect after this rebuild+redeploy.
# (Legacy Pages path was: ../tools/cf-sync-env.sh then retry the Pages deployment.)
```
