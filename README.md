...
- **`index.html`** ships full base metadata: canonical, Open Graph + Twitter cards (1200×630 `og-card.jpg`), `theme-color`, and JSON-LD (`WebSite` + `Organization`).
- **Per-route meta** via the `useSeo` hook (`src/hooks/useSeo.ts`) — unique `<title>`, description, and canonical per page for JS-rendering crawlers (Googlebot).
- **`public/sitemap.xml`** lists the indexable routes; **`public/robots.txt`** disallows transient/parameterized routes (`/claim`, `/receipt`, `/fund`, `/embed/`).
- **`public/_headers`** sets `immutable`, 1-year cache on Vite content-hashed `/assets/*` (Cloudflare Pages).
- Submit `https://cyrusthegreat.dev/sitemap.xml` in Google Search Console after deploy.
